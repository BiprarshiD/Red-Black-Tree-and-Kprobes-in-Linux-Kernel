...
{
	sprint_symbol(KERN_INFO "EIP is at %s\n", regs->ip);
...
void handler_post(struct kprobe *kp, struct pt_regs *regs, unsigned long flags){
	...
	temp_length += sprintf(output+temp_length,"address = 0x%lx, ",addr);
...
static int kp_register(int opc_val){
	...
	if (opc_val == 0) {
		printk(KERN_INFO "KPROBE : OPC_VAL = %d . Probing rb_write \n",opc_val);
		
		printk(KERN_INFO "KPROBE lookup : %pi \n", (void **)(kallsyms_lookup_name("rb_write")));
		printk(KERN_INFO "KPROBE lookup offset : %pi \n", (void **)(kallsyms_lookup_name("rb_write")));
		kp_kpt.addr = (kprobe_opcode_t *)(void **)kallsyms_lookup_name("rb_write"); // read regs->r12
	} else {
...
ssize_t kp_write_handler(struct file *file, const char *buf, size_t count, loff_t *ppos){
	...
	//wr_err = copy_from_user(input,buf,count);

	/*if(wr_err!=0){
	printk(KERN_INFO "KPROBE : Error while writing data.\n");
	return -1;
	}*/
	
	while (count) {
	get_user(input[current_write_pointer], buf++);
	count--;
	if(count){
